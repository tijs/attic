...
public struct MigrationRunner: Sendable {
    ...
    /// Delegates to `runThumbnailCleanupPhaseImpl` in
    /// `ThumbnailCleanupPhase.swift`. Keeps the runner under SwiftLint's
    /// file-length budget without exposing private state cross-file.
    private func runThumbnailCleanupPhase(
        manifest: Manifest,
        dryRun: Bool,
        confirmCleanup: CleanupConfirmHandler,
    ) async throws -> ThumbnailCleanupResult {
        try await runThumbnailCleanupPhaseImpl(
            s3: s3,
